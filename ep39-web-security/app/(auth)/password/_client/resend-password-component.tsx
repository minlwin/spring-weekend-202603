...
export default function ResendPasswordFormComponent() {
    const form = useForm({
    ...
    function requestForgotPassword(form : ForgotPasswordForm) {
        safeCall(async () => {
            await action.resendOtpRequest(form)
        })
    }
    ...
    return (
        <form onSubmit={form.handleSubmit(requestForgotPassword)} className="space-y-4">
            <header>
                ...
                <h3 className="text-gray-600">Resend OTP Code to your email to reset your password.</h3>
            </header>
